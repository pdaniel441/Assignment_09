...
class FileIO:
    ...
    def save_inventory(file_name: list, lst_Inventory: list) -> None:
        ...
        tracks_file_name = file_name[1]
        try:
            with open(cd_file_name,'w') as file:
                for cd in lst_Inventory:
                    file.write(cd.get_record())
           
            # TODone add code to save track data to file
            with open(tracks_file_name, 'w') as file:
                for disc in lst_Inventory:
                    album_tracks = disc.cd_tracks  # list of tracks contained in CD Object
                    album_id = disc.cd_id            # identifier for CD Object
                    for item in album_tracks:        # Loop through Track Objects for CD
                        if item is not None:
                            file.write('{},{}'.format(album_id, item.get_record()))
    ...
    def load_inventory(file_name: list) -> list:
        ...
        try:
            ...
            with open(tracks_file_name,'r') as file:
                for line in file:
                    data = line.strip().split(',')
                    data[0] = int(data[0])
                    tmp_cd = PC.DataProcessor.select_cd(lst_Inventory, data[0])     # Use Data processor to select CD that Track is assigned to
                    tmp_track = DC.Track(int(data[1]),data[2],data[3])   # Create Track Object with remaining data from line in File
                    tmp_cd.add_track(tmp_track)     # Add Track Object to appropriate CD
